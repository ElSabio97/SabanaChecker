...
if not st.session_state.df.empty:
    ...
    if alias_input:
        ...
        if matches and matches[1] >= 50:
            ...
            # --- Nueva lógica para intercambio de IM ---
            st.subheader("Intercambio de Instrucción en Mantenimiento (IM)")

            with st.form(key="search_form_im"):
                # Selector de fecha única (IM que el usuario quiere dar)
                user_im_dates = [col for col in date_columns if "IM" in str(user_row[col])]
                if user_im_dates:
                    selected_im_date = st.selectbox(
                        "Selecciona la fecha de tu IM que quieres dar:",
                        options=user_im_dates,
                        format_func=lambda x: x,
                        key="im_date"
                    )
                else:
                    st.warning("No tienes IM en fechas futuras.")
                    selected_im_date = None

                # Selector de fechas múltiples (IM que el usuario está dispuesto a tomar)
                available_im_dates = st.multiselect(
                    "Selecciona fechas en las que estás dispuesto a hacer el IM del compañero:",
                    options=date_columns,
                    format_func=lambda x: x,
                    key="available_im_dates"
                )

                # Botón "Buscar" dentro del formulario para IM
                search_im_button = st.form_submit_button(label="Buscar compañeros para IM")

            # Procesar solo si se presiona "Buscar" para IM
            if search_im_button and selected_im_date and available_im_dates:
                ...
                if not potential_im_swaps.empty:
                    ...
                    if im_swap_candidates:
                        st.subheader("Posibles compañeros para intercambio de IM:")
                        for candidate in im_swap_candidates:
                            st.write(f"**Alias**: {candidate['Alias']}")
                            st.write(f"- Libre en: {candidate['Available on']}")
                            st.write(f"- IM disponibles para cubrir:")
                            for date, activity in candidate['Activities'].items():
                                st.write(f"  - {date}: {activity}")
                            st.write("---")
                    else:
                        st.warning("No hay compañeros con IM en las fechas seleccionadas.")
                else:
